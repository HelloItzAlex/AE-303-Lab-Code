...
%% Data Processing
u_Bar = mean(u, 3, 'omitnan');
v_Bar = mean(v, 3, 'omitnan');
sigmaUprimeSQ= zeros(winsize);
SigmaVPrimeSQ= zeros(winsize);
SigmaUPrime_VPrime= zeros(winsize);
N=0;
for k=1:300
uPrime=u_raw(:, :, k) - u_Bar;
vPrime=v_raw(:, :, k) - v_Bar;
sigmaUprimeSQ= sigmaUprimeSQ+uPrime.*uPrime;
SigmaVPrimeSQ= SigmaVPrimeSQ +vPrime.*vPrime;
SigmaUPrime_VPrime= SigmaUPrime_VPrime+ uPrime.* vPrime;
N = N+1;
end
uprimeSQbar= sigmaUprimeSQ /N;
vprimeSQbar= SigmaVPrimeSQ /N;
uPrime_VPrime_Bar= SigmaUPrime_VPrime /N;
% Calculate derivatives and fluid dynamics parameters
[gradUx, gradUy] = gradient(u_Bar);
[gradVx, gradVy] = gradient(v_Bar);
